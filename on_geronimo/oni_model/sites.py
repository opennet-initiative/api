"""
The API automatically assembles location-specific "sites".
These sites are determined dynamically:
    * look for multiple APs being close together
    * define these as a site
    * adjust the allowed "size" (radius) of that site accoording to the number of contained nodes
    * re-evaluate the members of the site based on the new size

The result of this site detection process is not deterministic, but it should work fairly well.

Sites are used for improved map visualizations: multiple connections between two sites (e.g.
all APs bridged over a fiber connection) can be reduced/simplified based on the understanding of
sites.

The update of the site status is triggered periodically via a cron job.
"""
...
class SiteUpdater:
    ...
    def __init__(self, status_output_func=None):
        if status_output_func is None:
            self._log_output = logging.getLogger().info
        else:
            self._log_output = status_output_func
    ...
    def _remove_dislocated_accesspoints_from_sites(self):
        ...
        for site in AccessPointSite.objects.all():
            for ap in site.accesspoints.filter(position__isnull=True):
                self._log_output("Removing {} from {} due to its missing position"
                                 .format(ap, site))
                ap.site = None
                ap.save()
        ...
        for site in AccessPointSite.objects.all():
            ...
            if site_position:
                for ap in site.accesspoints.filter(
                    ...
                    self._log_output("Removing {} from {} due to its distance: {}"
    ...
    def _remove_underpopulated_sites(self):
        ...
        for site in (AccessPointSite.objects
            ...
            self._log_output("Removing underpopulated {}".format(site))
            site.delete()
        ...
        while unprocessed_sites:
            ...
            for other_site in overlapping_sites:
                if site.accesspoints.count() > other_site.accesspoints.count():
                    ...
                    break_loop_now = True
                self._log_output("Removing {} due to spatial conflict with {}"
        ...
        for site in AccessPointSite.objects.all():
            ...
            if site_position:
                ...
                for new_ap in (
                    ...
                    self._log_output("Adding {} to {}".format(new_ap, site))
                    new_ap.site = site
                    new_ap.save()
        ...
        while unprocessed_aps:
            ...
            if len(neighbour_aps) >= MINIMUM_SITE_POPULATION:
                ...
                for new_ap in neighbour_aps:
                    ...
                    new_ap.save()
                self._log_output("Created: {}".format(new_site))
